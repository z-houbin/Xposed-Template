...
package com.example.xposed.library.shell;
...
public class Shell {
    ...
    /**
     * <p>Runs commands using the supplied shell, and returns the type.</p>
     *
     ...
     * @param shell    The shell to use for executing the commands
     * @param commands The commands to execute
     * @param env      List of all environment variables (in 'key=value' format) or null for defaults
     * @return Output of the commands, or null in case of an error
     */
    @WorkerThread
     ...
     *
     * @param command     The name of the program to execute. E.g. "su" or "sh".
    ...
    public static class SH {
         ...
         * {@link Interactive}, {@link Console} or {@link #getConsole()}</p>
         *
    ...
    public static class SU {
         ...
         *
         * <p>This function caches its type to improve performance on multiple calls</p>
         *
         ...
         * This is can be an expensive operation, consider caching the type.
         *
        ...
        /**
         * Detect if SELinux is set to enforcing, caches type
         *
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                    ...
                    if (f.exists()) {
                        ...
                        try {
                            ...
                            // we might not be allowed to readConfig, thanks SELinux
                        } finally {
                            if (is != null) {
                                try {
                                    is.close();
    ...
    /**
     * Command type callback, notifies the recipient of the completion of a command block, including the (last) exit
     * code, and the full output
     */
    public interface OnCommandResultListener extends ShellExitCode {

        /**
         * <p>Command type callback</p>
         *
         ...
         * callback may pause the native process or even type in a deadlock</p>
         *
         * <p>See {@link Interactive} for threading details</p>
         *
    ...
    public static class Builder {
         ...
         *
         * <p>See {@link Interactive} for further details on threading and handlers</p>
         *
        ...
        /**
         * Set if error output should be appended to command block type output
         *
         ...
         *
         * <p>The thread on which the callback executes is dependent on various factors, see {@link Interactive}
        ...
        /**
         * Construct a {@link Interactive} instance, try to start the shell, and call onCommandResultListener to
    ...
    /**
     * <p>An interactive shell - initially created with {@link Builder} - that executes blocks of commands you
     ...
     * -lR /'</em> will probably have you run out of memory when using a {@link OnCommandResultListener}. A
     * work-around is to not supply this callback, but using (only)
     * {@link Builder#setOnStdoutLineListener(StreamGobbler.OnLineListener)}. This way, an internal buffer will
     ...
     * using  {@link Builder#setHandler(Handler)} if needed. If you do not supply a custom Handler - unless you set
     * {@link Builder#setAutoHandler(boolean)} to false - a Handler will be auto-created if the thread used for
     ...
     *
     * <p>The main thread must certainly have a Looper, thus if you call {@link Builder#open()} from the main
     ...
     *
     * <p>Background threads usually do <em>not</em> have a Looper, so calling {@link Builder#open()} from such a
     * background thread will (by default) type in all the callbacks being executed in one of the gobbler threads.
    ...
    public static class Interactive {
        ...
        private volatile boolean idle = true; // readConfig/write only synchronized
         ...
         * @param command        Command to execute
         * @param code           User-defined value passed back to the callback
         * @param resultListener Callback to be called on completion
         */
         ...
         * it and also returns the type code on completion.</p>
         *
         * <p>The thread on which the callback executes is dependent on various factors, see {@link Interactive}
         * for further details</p>
         *
         ...
         * @param commands       Commands to execute
         * @param code           User-defined value passed back to the callback
         * @param resultListener Callback to be called on completion (of all commands)
         */
    ...
    public static class Console implements Closeable {
        ...
        /**
         * Executes the commands in the shell and wait for its termination and return the type.
         *
         * @param commands The commands to execute.
        ...
        public interface OnCloseListener {
             ...
             *
             * @param console The {@link Console} holding the interactive shell that was closed.
             */
        ...
        public static class Builder {
            ...
            /**
             * Set if error output should be readConfig and returned to the {@link CommandResult}.
             *
             * @param wantStderr {@code true} to readConfig error output
             * @return This Builder object for method chaining
             */
            public Builder setWantStderr(boolean wantStderr) {
                this.wantStderr = wantStderr;
             ...
             *
             * @param onCloseListener The {@link OnCloseListener}
             * @return This Builder object for method chaining
             */
             ...
             * @throws ShellNotFoundException If the shell is not an executable or could not be opened.
             */
